...
function TrendingCRUD() {
  ...
  const [currentItem, setCurrentItem] = useState({ name: '', rating: 0, time: '', desc: '', starring: '', genres: '', tags: '', cover: '', date: '' });
  ...
  const handleShowModal = (type, item = null) => {
    setModalType(type);
    setCurrentItem(item || { name: '', rating: 0, time: '', desc: '', starring: '', genres: '', tags: '', cover: '', date: '' });
    setShowModal(true);
  };

  const handleCloseModal = () => {
    setShowModal(false);
    setCurrentItem({ name: '', rating: 0, time: '', desc: '', starring: '', genres: '', tags: '', cover: '', date: '' });
  };
  ...
  return (
    ...
    <div className="trending-crud-container">
      ...
      <div className="trending-grid">
        {trending.map((item) => (
          <div key={item.id} className="trending-item">
            ...
            <p><strong>Starring:</strong> {item.starring}</p>
            <p><strong>Genres:</strong> {item.genres}</p>
            <p><strong>Tags:</strong> {item.tags}</p>
            <p><strong>Release Date:</strong> {item.date}</p>
            <div className="item-buttons">
              <Button variant="warning" onClick={() => handleShowModal('update', item)}>Update</Button>
      ...
      <Button variant="success" className="add-button" onClick={() => handleShowModal('add')}>Add New Item</Button>

      {/* Modal for Add/Update */}
      <Modal show={showModal} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{modalType === 'add' ? 'Add New Item' : 'Update Item'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            ...
            <Form.Group controlId="formRating">
              ...
              <Form.Control
                ...
                onChange={(e) => setCurrentItem({ ...currentItem, rating: parseFloat(e.target.value) })}
              />
            </Form.Group>
            ...
            <Form.Group controlId="formStarring">
              ...
              <Form.Control
                ...
                onChange={(e) => setCurrentItem({ ...currentItem, starring: e.target.value })}
              />
            </Form.Group>
            <Form.Group controlId="formGenres">
              ...
              <Form.Control
                ...
                onChange={(e) => setCurrentItem({ ...currentItem, genres: e.target.value })}
              />
            </Form.Group>
            ...
            <Form.Group controlId="formCover">
              ...
              <Form.Control
                ...
                onChange={(e) => setCurrentItem({ ...currentItem, cover: e.target.value })}
              />
            </Form.Group>
